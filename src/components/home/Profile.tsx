...
export default function Profile({ author, social, features, researchInterests }: ProfileProps) {
    ...
    useEffect(() => {
        ...
        const userHasLiked = localStorage.getItem('yzgeng-website-user-liked');
    ...
    const handleLike = () => {
        ...
        if (newLikedState) {
            localStorage.setItem('yzgeng-website-user-liked', 'true');
            setShowThanks(true);
            setTimeout(() => setShowThanks(false), 2000);
        } else {
            localStorage.removeItem('yzgeng-website-user-liked');
            setShowThanks(false);
        }
    };
